...
IF (M%N_WALL_CELLS_DIM==0)      M%N_WALL_CELLS_DIM     = N_SLOTS
IF (M%N_THIN_WALL_CELLS_DIM==0) M%N_THIN_WALL_CELLS_DIM= 10
IF (M%N_BOUNDARY_COORD_DIM==0)  M%N_BOUNDARY_COORD_DIM = N_SLOTS
IF (M%N_BOUNDARY_PROP1_DIM==0)  M%N_BOUNDARY_PROP1_DIM = N_SLOTS
IF (M%N_BOUNDARY_PROP2_DIM==0)  M%N_BOUNDARY_PROP2_DIM = N_SLOTS
IF (M%N_BOUNDARY_RADIA_DIM==0)  M%N_BOUNDARY_RADIA_DIM = N_SLOTS
IF (M%N_BOUNDARY_ONE_D_DIM==0)  M%N_BOUNDARY_ONE_D_DIM = N_SLOTS
IF (M%N_BOUNDARY_THR_D_DIM==0)  M%N_BOUNDARY_THR_D_DIM = 10

IF (.NOT.ALLOCATED(M%WALL))      ALLOCATE(M%WALL(0:M%N_WALL_CELLS_DIM))
IF (.NOT.ALLOCATED(M%THIN_WALL)) ALLOCATE(M%THIN_WALL(1:M%N_THIN_WALL_CELLS_DIM))

IF (.NOT.ALLOCATED(M%BOUNDARY_COORD)) ALLOCATE(M%BOUNDARY_COORD(1:M%N_BOUNDARY_COORD_DIM))
IF (.NOT.ALLOCATED(M%BOUNDARY_PROP1)) ALLOCATE(M%BOUNDARY_PROP1(1:M%N_BOUNDARY_PROP1_DIM))
IF (.NOT.ALLOCATED(M%BOUNDARY_PROP2)) ALLOCATE(M%BOUNDARY_PROP2(1:M%N_BOUNDARY_PROP2_DIM))
IF (.NOT.ALLOCATED(M%BOUNDARY_RADIA)) ALLOCATE(M%BOUNDARY_RADIA(1:M%N_BOUNDARY_RADIA_DIM))
IF (.NOT.ALLOCATED(M%BOUNDARY_ONE_D)) ALLOCATE(M%BOUNDARY_ONE_D(1:M%N_BOUNDARY_ONE_D_DIM))
IF (.NOT.ALLOCATED(M%BOUNDARY_THR_D)) ALLOCATE(M%BOUNDARY_THR_D(1:M%N_BOUNDARY_THR_D_DIM))

IF (.NOT.ALLOCATED(M%BOUNDARY_COORD_OCCUPANCY)) THEN
   ALLOCATE(M%BOUNDARY_COORD_OCCUPANCY(1:M%N_BOUNDARY_COORD_DIM))
   M%BOUNDARY_COORD_OCCUPANCY = 0
   M%NEXT_AVAILABLE_BOUNDARY_COORD_SLOT = 1
ENDIF
IF (.NOT.ALLOCATED(M%BOUNDARY_PROP1_OCCUPANCY)) THEN
   ALLOCATE(M%BOUNDARY_PROP1_OCCUPANCY(1:M%N_BOUNDARY_PROP1_DIM))
   M%BOUNDARY_PROP1_OCCUPANCY = 0
   M%NEXT_AVAILABLE_BOUNDARY_PROP1_SLOT = 1
ENDIF
IF (.NOT.ALLOCATED(M%BOUNDARY_PROP2_OCCUPANCY)) THEN
   ALLOCATE(M%BOUNDARY_PROP2_OCCUPANCY(1:M%N_BOUNDARY_PROP2_DIM))
   M%BOUNDARY_PROP2_OCCUPANCY = 0
   M%NEXT_AVAILABLE_BOUNDARY_PROP2_SLOT = 1
ENDIF
IF (.NOT.ALLOCATED(M%BOUNDARY_RADIA_OCCUPANCY)) THEN
   ALLOCATE(M%BOUNDARY_RADIA_OCCUPANCY(1:M%N_BOUNDARY_RADIA_DIM))
   M%BOUNDARY_RADIA_OCCUPANCY = 0
   M%NEXT_AVAILABLE_BOUNDARY_RADIA_SLOT = 1
ENDIF
IF (.NOT.ALLOCATED(M%BOUNDARY_ONE_D_OCCUPANCY)) THEN
   ALLOCATE(M%BOUNDARY_ONE_D_OCCUPANCY(1:M%N_BOUNDARY_ONE_D_DIM))
   M%BOUNDARY_ONE_D_OCCUPANCY = 0
   M%NEXT_AVAILABLE_BOUNDARY_ONE_D_SLOT = 1
ENDIF
IF (.NOT.ALLOCATED(M%BOUNDARY_THR_D_OCCUPANCY)) THEN
   ALLOCATE(M%BOUNDARY_THR_D_OCCUPANCY(1:M%N_BOUNDARY_THR_D_DIM))
   M%BOUNDARY_THR_D_OCCUPANCY = 0
   M%NEXT_AVAILABLE_BOUNDARY_THR_D_SLOT = 1
ENDIF

END SUBROUTINE ALLOCATE_BOUNDARY_TYPES
...
ELSEIF (PRESENT(THIN_WALL_INDEX)) THEN
   ...
   CALL PACK_THIN_WALL(NM,OS_DUMMY,TW,SURF_INDEX,TW%N_REALS,TW%N_INTEGERS,UNPACK_IT=.FALSE.,COUNT_ONLY=.TRUE.)
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN
   ...
   IF (ALLOCATED(M%CFACE(CFACE_INDEX)%ZZ_G)) DEALLOCATE(M%CFACE(CFACE_INDEX)%ZZ_G)
   ALLOCATE(M%CFACE(CFACE_INDEX)%ZZ_G(N_TRACKED_SPECIES))
   M%CFACE(CFACE_INDEX)%ZZ_G(1:N_TRACKED_SPECIES) = SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0
   IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) CALL ALLOCATE_BOUNDARY_COORD_ARRAYS
   IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL ALLOCATE_BOUNDARY_ONE_D_ARRAYS
   IF (SF%INCLUDE_BOUNDARY_PROP1_TYPE) CALL ALLOCATE_BOUNDARY_PROP1_ARRAYS
   IF (SF%INCLUDE_BOUNDARY_PROP2_TYPE) CALL ALLOCATE_BOUNDARY_PROP2_ARRAYS
...
IF (BC_INDEX>M%N_BOUNDARY_COORD_DIM) THEN  ! There are no open slots for boundary coordinates
   ...
   DUMMY(1:M%N_BOUNDARY_COORD_DIM) = M%BOUNDARY_COORD_OCCUPANCY(1:M%N_BOUNDARY_COORD_DIM)
...
IF (OD_INDEX>M%N_BOUNDARY_ONE_D_DIM) THEN  ! There are no open slots for boundary coordinates
   ...
   DUMMY(1:M%N_BOUNDARY_ONE_D_DIM) = M%BOUNDARY_ONE_D_OCCUPANCY(1:M%N_BOUNDARY_ONE_D_DIM)
...
IF (TD_INDEX>M%N_BOUNDARY_THR_D_DIM) THEN  ! There are no open slots for boundary coordinates
   ALLOCATE(TD_DUMMY(1:M%N_BOUNDARY_THR_D_DIM+N_NEW_STORAGE_SLOTS))
   TD_DUMMY(1:M%N_BOUNDARY_THR_D_DIM) = M%BOUNDARY_THR_D(1:M%N_BOUNDARY_THR_D_DIM)
   CALL MOVE_ALLOC(TD_DUMMY,M%BOUNDARY_THR_D)
   ALLOCATE(DUMMY(1:M%N_BOUNDARY_THR_D_DIM+N_NEW_STORAGE_SLOTS)) ; DUMMY = 0
   DUMMY(1:M%N_BOUNDARY_THR_D_DIM) = M%BOUNDARY_THR_D_OCCUPANCY(1:M%N_BOUNDARY_THR_D_DIM)
...
IF (B1_INDEX>M%N_BOUNDARY_PROP1_DIM) THEN  ! There are no open slots for boundary coordinates
   ALLOCATE(B1_DUMMY(1:M%N_BOUNDARY_PROP1_DIM+N_NEW_STORAGE_SLOTS))
   B1_DUMMY(1:M%N_BOUNDARY_PROP1_DIM) = M%BOUNDARY_PROP1(1:M%N_BOUNDARY_PROP1_DIM)
   CALL MOVE_ALLOC(B1_DUMMY,M%BOUNDARY_PROP1)
   ALLOCATE(DUMMY(1:M%N_BOUNDARY_PROP1_DIM+N_NEW_STORAGE_SLOTS)) ; DUMMY = 0
   DUMMY(1:M%N_BOUNDARY_PROP1_DIM) = M%BOUNDARY_PROP1_OCCUPANCY(1:M%N_BOUNDARY_PROP1_DIM)
...
IF (B2_INDEX>M%N_BOUNDARY_PROP2_DIM) THEN  ! There are no open slots for boundary coordinates
   ALLOCATE(B2_DUMMY(1:M%N_BOUNDARY_PROP2_DIM+N_NEW_STORAGE_SLOTS))
   B2_DUMMY(1:M%N_BOUNDARY_PROP2_DIM) = M%BOUNDARY_PROP2(1:M%N_BOUNDARY_PROP2_DIM)
   CALL MOVE_ALLOC(B2_DUMMY,M%BOUNDARY_PROP2)
   ALLOCATE(DUMMY(1:M%N_BOUNDARY_PROP2_DIM+N_NEW_STORAGE_SLOTS)) ; DUMMY = 0
   DUMMY(1:M%N_BOUNDARY_PROP2_DIM) = M%BOUNDARY_PROP2_OCCUPANCY(1:M%N_BOUNDARY_PROP2_DIM)
...
IF (BR_INDEX>M%N_BOUNDARY_RADIA_DIM) THEN  ! There are no open slots for boundary coordinates
   ...
   DUMMY(1:M%N_BOUNDARY_RADIA_DIM) = M%BOUNDARY_RADIA_OCCUPANCY(1:M%N_BOUNDARY_RADIA_DIM)
...
IF (LPC%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,OS,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (LPC%INCLUDE_BOUNDARY_PROP1_TYPE) CALL PACK_BOUNDARY_PROP1(NM,IC,RC,LC,OS,B1_INDEX,UNPACK_IT,COUNT_ONLY)
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC),WC%BACK_INDEX,UNPACK_IT)
...
! Pack or unpack the real values

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),WC%DUNDT,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),WC%Q_LEAK,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),WC%V_DEP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),WC%VEL_ERR_NEW,UNPACK_IT)
...
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,OS,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROP1_TYPE) CALL PACK_BOUNDARY_PROP1(NM,IC,RC,LC,OS,B1_INDEX,UNPACK_IT,COUNT_ONLY)
...
SUBROUTINE PACK_THIN_WALL(NM,OS,TW,SURF_INDEX,RC,IC,UNPACK_IT,COUNT_ONLY)
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC),TW%BACK_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC),TW%BACK_MESH,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC),TW%BACK_SURF,UNPACK_IT)
...
INTEGER, INTENT(IN) :: NM,SURF_INDEX
LOGICAL, INTENT(IN) :: UNPACK_IT,COUNT_ONLY
INTEGER, INTENT(OUT) :: RC,IC,LC
INTEGER :: I1,I2,BC_INDEX,OD_INDEX,B1_INDEX,B2_INDEX,BR_INDEX
TYPE(CFACE_TYPE), POINTER :: CFA
TYPE(STORAGE_TYPE), POINTER :: OS
TYPE(SURFACE_TYPE), POINTER :: SF
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC),CFA%BACK_MESH,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC),CFA%BACK_INDEX,UNPACK_IT)
...
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),CFA%VEL_ERR_NEW,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),CFA%V_DEP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),CFA%Q_LEAK,UNPACK_IT)
...
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),CFA%RHO_G,UNPACK_IT)
...
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2) , CFA%ZZ_G(1:I2-I1+1) , UNPACK_IT)
RC = I2
...
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),B1%T_SCALE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC),B1%Q_SCALE,UNPACK_IT)
...
CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,CELL_COUNT_INTEGERS,COUNTS,DISPLS,MPI_INTEGER,MPI_COMM_WORLD,IERR)
CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,CELL_COUNT_LOGICALS,COUNTS,DISPLS,MPI_INTEGER,MPI_COMM_WORLD,IERR)
...
DO NM=1,NMESHES
   ...
   M => MESHES(NM)
   CALL MPI_BCAST(M%CELL_INDEX,SIZE(M%CELL_INDEX),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
   CALL MPI_BCAST(M%CELL_INTEGERS,SIZE(M%CELL_INTEGERS),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
   CALL MPI_BCAST(M%CELL_LOGICALS,SIZE(M%CELL_LOGICALS),MPI_LOGICAL,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
ENDDO
...
DX3 => MESHES(NM)%DZ
SELECT CASE (AXIS)
CASE(IAXIS); DX1 => MESHES(NM)%DXN
CASE(JAXIS); DX2 => MESHES(NM)%DYN
CASE(KAXIS); DX3 => MESHES(NM)%DZN
END SELECT
...
IF (PRESENT(WALL_INDEX)) THEN
   ONE_D => M%BOUNDARY_ONE_D(M%WALL(WALL_INDEX)%OD_INDEX)
   SF => SURFACE(M%WALL(WALL_INDEX)%SURF_INDEX)
ELSEIF (PRESENT(LAGRANGIAN_PARTICLE_INDEX)) THEN
   ONE_D => M%BOUNDARY_ONE_D(M%LAGRANGIAN_PARTICLE(LAGRANGIAN_PARTICLE_INDEX)%OD_INDEX)
...
IF (SF%EMBER_POWER_MEAN<0._EB) RETURN
...
POWER = (M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_RAD_OUT-M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_RAD_IN - M%BOUNDARY_PROP1(LP%B1_INDEX)%Q_CON_F)*&
         0.001_EB*M%BOUNDARY_PROP1(LP%B1_INDEX)%AREA*LP%PWT

! Compute the CDF of the POWER

F_N = 0.5_EB*(1._EB + ERF((POWER-SF%EMBER_POWER_MEAN)/(SR2*SF%EMBER_POWER_SIGMA)))
