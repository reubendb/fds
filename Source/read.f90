...
CHARACTER(LABEL_LENGTH) :: FUEL_RADCAL_ID='METHANE'
...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE,ALLOW_SURFACE_PARTICLES, &
                ...
                C_SMAGORINSKY,C_VREMAN,C_WALE,DEPOSITION,&
                ...
                MPI_TIMEOUT,NEIGHBOR_SEPARATION_DISTANCE,NORTH_BEARING,&
                ...
                SC,SHARED_FILE_SYSTEM,SIMULATION_MODE,SMOKE3D_16,SMOKE_ALBEDO,SOLID_PHASE_ONLY,SOOT_DENSITY,SOOT_OXIDATION,&
...
SELECT CASE (TRIM(LES_FILTER_TYPE))
   CASE DEFAULT
      LES_FILTER_WIDTH_TYPE = MEAN_LES_FILTER_WIDTH
   CASE ('MAX')
      LES_FILTER_WIDTH_TYPE = MAX_LES_FILTER_WIDTH
END SELECT
...
IF (STRATIFICATION) THEN
   ...
   IF (ABS(L)>1.E-10_EB) THEN
      ...
      RRP2%NUMBER_DATA_POINTS = N_MO_PTS
      ALLOCATE(RRP%INDEPENDENT_DATA(N_MO_PTS))
      ALLOCATE(RRP%DEPENDENT_DATA(N_MO_PTS))
      ALLOCATE(RRP2%INDEPENDENT_DATA(N_MO_PTS))
      ALLOCATE(RRP2%DEPENDENT_DATA(N_MO_PTS))
      ...
      DO I=1,N_MO_PTS
         ...
         IF (SPEED>TWO_EPSILON_EB) THEN
            RRP2%DEPENDENT_DATA(I) = MAX(0._EB,U/SPEED)
         ELSE
            RRP2%DEPENDENT_DATA(I) = 0._EB
         ENDIF
      ENDDO
      RAMP_TMP0_Z = 'RSRVD TEMPERATURE PROFILE'
      CALL GET_RAMP_INDEX(RAMP_TMP0_Z,'PROFILE',I_RAMP_TMP0_Z)
...
NAMELIST /DUMP/ CFL_FILE,CLIP_RESTART_FILES,COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,DEVC_COLUMN_LIMIT,&
                ...
                DT_RADF,DT_RESTART,DT_SL3D,DT_SLCF,DT_SMOKE3D,DT_UVW,&
...
USE PROPERTY_DATA, ONLY: GAS_PROPS,FED_PROPS,CHECK_PREDEFINED,ELEMENT
...
PRIMITIVE_LOOP: DO N1=1,N_SPECIES - N_COPY_PRIMITIVE
   ...
   CALL GAS_PROPS(SS%PROP_ID,SS%SIG,SS%EPSK,SS%PR_USER,SS%MW,SS%FORMULA,SS%LISTED,SS%ATOMS,SS%H_F_LISTED,SS%RADCAL_ID)
   ...
   IF (SS%REFERENCE_ENTHALPY > -1.E21_EB .OR. SS%H_F > -1.E21_EB) SS%EXPLICIT_H_F = .TRUE.
   ...
   IF (SIMPLE_CHEMISTRY) THEN
      DO NR=1,N_REACTIONS
         ...
         ENDIF
      ENDDO
      IF (TRIM(ID)=='SOOT') SS%MW = SOOT_H_FRACTION_GLOBAL*ELEMENT(1)%MASS+(1._EB-SOOT_H_FRACTION_GLOBAL)*ELEMENT(6)%MASS
   ENDIF
      ...
      ELSE
         ...
         IF (SPECIES_MIXTURE(N_DEFINE(N1))%SINGLE_SPEC_INDEX > 0) THEN
            IF (SPECIES(SPECIES_MIXTURE(N_DEFINE(N1))%SINGLE_SPEC_INDEX)%CONDENSABLE) THEN
               ...
               SPECIES_MIXTURE(N_DEFINE(N1))%SINGLE_SPEC_INDEX=SPECIES_MIXTURE(N1-1)%SINGLE_SPEC_INDEX
...
DO N1 = 1, N_TRACKED_SPECIES
   IF (SPECIES_MIXTURE(N1)%SINGLE_SPEC_INDEX==O2_INDEX) NU_SOOT_OX(N1) = -(SS%ATOMS(1) * 0.25_EB + SS%ATOMS(6)) * MW_O2 / SS%MW
...
ELSE BACKGROUND_IF ! Mixture is fuel or products
   ...
   N_SIMPLE_CHEM_RXN: SELECT CASE (RN%N_SIMPLE_CHEMISTRY_REACTIONS)
      CASE (1) N_SIMPLE_CHEM_RXN
         ...
         RN%NU_H2O          = 0.5_EB*RN%H - 0.5_EB*(RN%NU_SOOT*SOOT_H_FRACTION_GLOBAL + RN%NU_HCN)
         IF (ABS(RN%NU_H2O) < TWO_EPSILON_EB) RN%NU_H2O = 0._EB
         RN%NU_CO2          = RN%C - RN%NU_CO - RN%NU_HCN - RN%NU_SOOT*(1._EB-SOOT_H_FRACTION_GLOBAL)
         IF (ABS(RN%NU_CO2) < TWO_EPSILON_EB) RN%NU_CO2 = 0._EB
         RN%NU_O2           = RN%NU_CO2 + 0.5_EB*(RN%NU_CO+RN%NU_H2O-RN%O)
         RN%NU_N2           = 0.5_EB*(RN%N - RN%NU_HCN)
         ...
         SPECIES(SOOT_INDEX)%ATOMS(1)=SOOT_H_FRACTION_GLOBAL
      ...
      CASE (2) N_SIMPLE_CHEM_RXN
         ...
         RN%NU_SOOT         = (RN%C-RN%NU_CO-RN%NU_HCN)/(1._EB-SOOT_H_FRACTION_GLOBAL)
         ...
         RN2%NU_H2O          = 0.5_EB*RN%H - 0.5_EB*(RN2%NU_SOOT*SOOT_H_FRACTION_GLOBAL + RN2%NU_HCN)
         IF (ABS(RN2%NU_H2O) < TWO_EPSILON_EB) RN%NU_H2O = 0._EB
         RN2%NU_CO2          = RN%C - RN2%NU_CO - RN2%NU_HCN - RN2%NU_SOOT*(1._EB-SOOT_H_FRACTION_GLOBAL)
         IF (ABS(RN2%NU_CO2) < TWO_EPSILON_EB) RN2%NU_CO2 = 0._EB
         RN2%NU_O2           = RN2%NU_CO2 + 0.5_EB*(RN2%NU_CO+RN2%NU_H2O-RN%O)
         RN2%NU_N2           = 0.5_EB*(RN%N - RN2%NU_HCN)
...
USE PROPERTY_DATA, ONLY: THERMO_TABLE_LIQUID
...
CALL THERMO_TABLE_LIQUID (1,C_P_L,H_V,TMP_REF,TMP_MELT,TMP_V,SS2_ID,FUEL2,DENSITY,MU_LIQUID,K_LIQUID,BETA_LIQUID)
...
SPEC_LOOP: DO N=1,N_SPECIES
   ...
   CALL THERMO_TABLE_LIQUID (1,CP_TEMP,H_V,TMP_REF,TMP_MELT,TMP_V,SS%PROP_ID,SS%ISFUEL,DENSITY,MU_LIQUID,K_LIQUID,BETA_LIQUID)
   ...
   LIQUID_IF: IF (CP_TEMP > 0._EB .OR. SS%TMP_MELT >= 0._EB .OR. SS%TMP_V > 0._EB .OR. SS%DENSITY_LIQUID > 0._EB .OR. &
      ...
      DO I=1,I_MAX_TEMP
         ...
         ELSE
            CALL THERMO_TABLE_LIQUID(I,SS%C_P_L(I),H_V,TMP_REF,TMP_MELT,TMP_V,SS%PROP_ID,SS%ISFUEL,DENSITY,MU_LIQUID,K_LIQUID,&
                                     BETA_LIQUID)
         ENDIF
...
! Loop through temperatures from 1 K to I_MAX_TEMP K to get temperature-specific gas properties.  Data from JANAF 4
...
NAMELIST /COMB/ CHECK_REALIZABILITY,COMPUTE_ADIABATIC_FLAME_TEMPERATURE,EXTINCTION_MODEL,&
                ...
                ODE_SOLVER,RICHARDSON_ERROR_TOLERANCE,SOOT_H_FRACTION,SUPPRESSION,TAU_CHEM,TAU_FLAME

ODE_SOLVER         = 'null'
SOOT_H_FRACTION = -1._EB
...
SOOT_H_FRACTION_GLOBAL = SOOT_H_FRACTION
...
INTEGER :: NR,NS,NS2,NFR,PRIORITY,PROD_COUNTER=0, NEW_REAC=0,N_SIMPLE_CHEMISTRY_REACTIONS,IZ
REAL(EB) :: SOOT_YIELD,CO_YIELD,HCN_YIELD,EPUMO2,A,LOWER_OXYGEN_LIMIT, &
            ...
            SOOT_H_FRACTION,N_T,NU(MAX_SPECIES),N_S(MAX_SPECIES),&
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (A < 0._EB .AND. E < 0._EB .AND. TRIM(SPEC_ID_NU(1))=='null' .AND. TRIM(EQUATION)=='null') THEN
   ...
   RN%MW_FUEL                   = MW_FUEL
   IF (SOOT_H_FRACTION_GLOBAL >= 0._EB .AND. SOOT_H_FRACTION >= 0._EB) THEN
      WRITE(MESSAGE,'(A)') 'WARNING: Multiple SOOT_H_FRACTION values defined. Last value defined will be used for all reactions.'
      IF (MY_RANK==0) WRITE(LU_ERR,'(A)') TRIM(MESSAGE)
   ENDIF
   IF (SOOT_H_FRACTION >= 0._EB) SOOT_H_FRACTION_GLOBAL = SOOT_H_FRACTION
   RN%N                         = N
   RN%N_T                       = N_T
...
N_REACTIONS = N_REACTIONS + NEW_REAC
...
IF (SOOT_H_FRACTION_GLOBAL < 0._EB) SOOT_H_FRACTION_GLOBAL = 0.1_EB
MW_SOOT = ELEMENT(6)%MASS * (1._EB-SOOT_H_FRACTION_GLOBAL) + ELEMENT(1)%MASS*SOOT_H_FRACTION_GLOBAL
      ...
      HOC_IF: IF (RN%HEAT_OF_COMBUSTION > -1.E21) THEN ! User specified heat of combustion
         IF (IDEAL) THEN
            ...
            RN2%HOC_COMPLETE = RN2%HOC_COMPLETE + ( &
                             ...
                             + RN2%NU_SOOT * SPECIES(CO2_INDEX)%H_F * (1._EB - SOOT_H_FRACTION_GLOBAL) * MW_CO2 &
                             + RN2%NU_SOOT * SPECIES(H2O_INDEX)%H_F * SOOT_H_FRACTION_GLOBAL * 0.5_EB * MW_H2O &
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   IF (MASSLESS) THEN
      ...
      LPC%INCLUDE_BOUNDARY_ONE_D_TYPE = .FALSE.
      DIAMETER = 0._EB
   ...
   LPC%NEW_PARTICLE_INCREMENT           = NEW_PARTICLE_INCREMENT
   LPC%PROP_ID                          = PROP_ID
...
REAL(EB), DIMENSION(MAX_REACTIONS) :: A,E,HEATING_RATE,PYROLYSIS_RANGE,HEAT_OF_REACTION, &
          ...
          GAS_DIFFUSION_DEPTH,NU_O2_CHAR,BETA_CHAR,MAX_REACTION_RATE
REAL(EB), DIMENSION(MAX_SPECIES,MAX_REACTIONS) :: NU_SPEC
REAL(EB) :: NU_PART(MAX_LPC,MAX_REACTIONS)
LOGICAL :: ADJUST_H,ALLOW_SHRINKING, ALLOW_SWELLING,ADD_MATL,FOUND
...
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,ADJUST_H,ALLOW_SHRINKING,ALLOW_SWELLING,BETA_CHAR,BOILING_TEMPERATURE,&
                ...
                MAX_REACTION_RATE,MW,N_O2,N_REACTIONS,N_S,N_T,NU_MATL,NU_O2_CHAR,NU_PART,NU_SPEC,PART_ID,&
                PYROLYSIS_RANGE,REFERENCE_ENTHALPY,REFERENCE_ENTHALPY_TEMPERATURE,&
                REFERENCE_RATE,REFERENCE_TEMPERATURE,REFRACTIVE_INDEX,&
                SPECIFIC_HEAT,SPECIFIC_HEAT_RAMP,SPEC_ID
   ...
   DO NR=1,ML%N_REACTIONS
      DO NN=1,ML%N_RESIDUE(NR)
         IF (ABS(ML%NU_RESIDUE(NN,NR))>TWO_EPSILON_EB .AND. MATERIAL(ML%RESIDUE_MATL_INDEX(NN,NR))%ADJUST_H) THEN
            IF (MATL_MATRIX_POINTER(ML%RESIDUE_MATL_INDEX(NN,NR))>0) CYCLE
            MATL_MATRIX_SIZE = MATL_MATRIX_SIZE + 1
...
LOGICAL :: ADIABATIC,BURN_AWAY,FREE_SLIP,NO_SLIP,CONVERT_VOLUME_TO_MASS,HORIZONTAL,DIRICHLET_FRONT,DIRICHLET_BACK
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            MATL_MASS_FRACTION(MAX_LAYERS,MAX_MATERIALS),CELL_SIZE,CELL_SIZE_FACTOR,MAX_PRESSURE,&
            ...
            REFERENCE_HEAT_FLUX,REFERENCE_HEAT_FLUX_TIME_INTERVAL,&
...
LOGICAL :: DEFAULT,VEG_LSET_SPREAD,VEG_LSET_TAN2,TGA_ANALYSIS,COMPUTE_EMISSIVITY,&
           COMPUTE_EMISSIVITY_BACK,HT3D,THERM_THICK,NORMAL_DIRECTION_ONLY
! Ember generating variables
REAL(EB) :: EMBER_GENERATION_HEIGHT(2)

NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BURN_AWAY,BURN_DURATION,&
                ...
                DT_INSERT,E_COEFFICIENT,EMBER_GENERATION_HEIGHT,&
                ...
                HEAT_TRANSFER_MODEL,HORIZONTAL,HRRPUA,HT3D,ID,IGNITION_TEMPERATURE,&
                ...
                RAMP_PART,RAMP_Q,RAMP_T,RAMP_TMP_BACK,RAMP_TMP_GAS_BACK,RAMP_TMP_GAS_FRONT,&
                RAMP_V,RAMP_V_X,RAMP_V_Y,RAMP_V_Z,REFERENCE_HEAT_FLUX,REFERENCE_HEAT_FLUX_TIME_INTERVAL,&
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (ANY(MOISTURE_FRACTION>TWO_EPSILON_EB) .OR. &
      ...
      LAYER_LOOP_2: DO NL=1,MAX_LAYERS
         ...
         IF (SURFACE_VOLUME_RATIO(NL)>TWO_EPSILON_EB .AND. THICKNESS(NL)<0._EB .AND. .NOT.HT3D) THEN
   ...
   SF%EMBER_GENERATION_HEIGHT = &
      (/MINVAL(EMBER_GENERATION_HEIGHT),MAXVAL(EMBER_GENERATION_HEIGHT)/)
   ...
   LAYER_LOOP: DO IL=1,MAX_LAYERS
      ...
      IF ((ADIABATIC.OR.NET_HEAT_FLUX<1.E12_EB.OR.ABS(CONVECTIVE_HEAT_FLUX)>TWO_EPSILON_EB) &
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (THICKNESS(IL)<=0._EB .AND. .NOT.HT3D .AND. MATL_ID(IL,1)/='null') THEN
   ...
   N_LIST = 0
   NAME_LIST = 'null'
   IF (SF%HT_DIM>1 .AND. .NOT.SF%NORMAL_DIRECTION_ONLY) THICKNESS(1) = 1.0_EB  ! Dummy thickness
   SF%THICKNESS  = 0._EB
...
REFERENCE_HEAT_FLUX_TIME_INTERVAL = 10.0
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (SF%REFERENCE_HEAT_FLUX >=0._EB) THEN
      ...
      RP => RAMPS(NR)
      RP2 => RAMPS(SF%RAMP(TIME_HEAT)%INDEX)
      RP = RP2
      ...
      HRRPUA_MLRPUA_IF: IF (SF%HRRPUA > 0._EB .OR. SF%MLRPUA > 0._EB) THEN
         H_OR_M_IF: IF (SF%HRRPUA>0._EB) THEN
            ...
            ELSE
               ...
               SF%RAMP(I_FUEL)    = SF%RAMP(TIME_HEAT)
            ENDIF
...
LOGICAL :: PROC_SURF_GRID,REMESH_LAYER(1:MAX_LAYERS)
...
SURF_GRID_LOOP: DO SURF_INDEX=0,N_SURF
   ...
   LAYER_LOOP: DO NL=1,SF%N_LAYERS
      ...
      CALL GET_N_LAYER_CELLS(SF%MIN_DIFFUSIVITY(NL),SF%SWELL_RATIO*SF%LAYER_THICKNESS(NL),SF%STRETCH_FACTOR(NL), &
                             SF%CELL_SIZE_FACTOR,SF%CELL_SIZE,SF%N_LAYER_CELLS_MAX(NL),SF%N_LAYER_CELLS(NL),&
...
LOGICAL :: EMBEDDED,THICKEN,THICKEN_LOC,PERMIT_HOLE,ALLOW_VENT,REMOVABLE,BNDF_FACE(-3:3),BNDF_OBST,OUTLINE,REJECT_OBST
NAMELIST /OBST/ ALLOW_VENT,BNDF_FACE,BNDF_OBST,BULK_DENSITY,COLOR,CTRL_ID,DEVC_ID,FYI,HEIGHT,ID,&
                LENGTH,MULT_ID,ORIENTATION,OUTLINE,OVERLAY,PERMIT_HOLE,&
                RADIUS,REMOVABLE,RGB,SHAPE,SURF_ID,SURF_ID6,SURF_IDS,TEXTURE_ORIGIN,THETA,THICKEN,&
         ...
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               DO IOR=-3,3
                  ...
                  IF (SF%HT_DIM>1 .AND. .NOT.SF%NORMAL_DIRECTION_ONLY) &
                     SF%LAYER_THICKNESS(1) = MAX(SF%LAYER_THICKNESS(1),MAXVAL(OB%UNDIVIDED_INPUT_LENGTH))
                  DO NNN=1,SF%N_MATL
...
NAMELIST /INIT/ BULK_DENSITY_FILE,CELL_CENTERED,CROWN_BASE_HEIGHT,CROWN_BASE_WIDTH,CTRL_ID,DB,DENSITY,DEVC_ID,&
                DIAMETER,DRY,DT_INSERT,DX,DY,DZ,HEIGHT,HRRPUV,ID,MASS_FRACTION,MASS_PER_TIME,MASS_PER_VOLUME,MULT_ID,NODE_ID,&
                N_PARTICLES,N_PARTICLES_PER_CELL,PART_ID,PARTICLE_WEIGHT_FACTOR,PATH_RAMP,RADIATIVE_FRACTION,&
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      IF (SETPOINT <1.E20_EB) WRITE_DEVC_CTRL = .TRUE.
      DV%LATCH             = LATCH
      DV%OUTPUT            = OUTPUT
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   MESH_LOOP_2: DO NM=1,NMESHES
      ...
      IF (DV%X1>-1.E5_EB) THEN
         ...
         SDV%I1 =   FLOOR( GINV(SDV%X1-M%XS,1,NM)*M%RDXI + TWO_EPSILON_EB ) + 1
         SDV%I2 = CEILING( GINV(SDV%X2-M%XS,1,NM)*M%RDXI - TWO_EPSILON_EB )
         SDV%J1 =   FLOOR( GINV(SDV%Y1-M%YS,2,NM)*M%RDETA + TWO_EPSILON_EB ) + 1
         SDV%J2 = CEILING( GINV(SDV%Y2-M%YS,2,NM)*M%RDETA - TWO_EPSILON_EB )
         SDV%K1 =   FLOOR( GINV(SDV%Z1-M%ZS,3,NM)*M%RDZETA + TWO_EPSILON_EB ) + 1
         SDV%K2 = CEILING( GINV(SDV%Z2-M%ZS,3,NM)*M%RDZETA - TWO_EPSILON_EB )
...
NAMELIST /PROF/ CELL_CENTERED,FORMAT_INDEX,FYI,ID,INIT_ID,IOR,QUANTITY,XYZ
...
REAL(EB) :: AGL_SLICE
INTEGER :: N,NN,NM,MESH_NUMBER,N_SLCF_O,NITER,ITER,VELO_INDEX,IG,GEOM_INDEX
LOGICAL :: VECTOR,CELL_CENTERED,FACE_CENTERED,DEBUG
      ...
      IF (.NOT.VECTOR .AND. ABS(AGL_SLICE+1._EB)<TWO_EPSILON_EB) THEN
         DO IG=1,N_GEOMETRY
            ...
            IF (XB(2)-XB(1)<TWO_EPSILON_EB) THEN
               IF(XB(1)>GEOMETRY(IG)%GEOM_BOX(HIGH_IND,IAXIS) .OR. XB(1)<GEOMETRY(IG)%GEOM_BOX( LOW_IND,IAXIS)) CYCLE
            ENDIF
            IF (XB(4)-XB(3)<TWO_EPSILON_EB) THEN
               IF(XB(3)>GEOMETRY(IG)%GEOM_BOX(HIGH_IND,JAXIS) .OR. XB(3)<GEOMETRY(IG)%GEOM_BOX( LOW_IND,JAXIS)) CYCLE
            ENDIF
            IF (XB(6)-XB(5)<TWO_EPSILON_EB) THEN
               IF(XB(5)>GEOMETRY(IG)%GEOM_BOX(HIGH_IND,KAXIS) .OR. XB(5)<GEOMETRY(IG)%GEOM_BOX( LOW_IND,KAXIS)) CYCLE
            ENDIF
...
USE OUTPUT_DATA
CHARACTER(255) :: UVWFILE,UVWFILE_NM
LOGICAL :: PER_MESH
INTEGER :: NM
NAMELIST /CSVF/ UVWFILE
...
CHARACTER(LABEL_LENGTH) :: WATER_VAPOR='WATER VAPOR',RADCAL_NAME
...
CALL GAS_PROPS(WATER_VAPOR,SIGMA,EPSOK,PR_GAS,MW,FORMULA,LISTED,ATOM_COUNTS,H_F,RADCAL_NAME)

H_G = 0
H_L = 0

DO I=1,I_MAX_TEMP
   CALL THERMO_TABLE_GAS (I,CP_G,WATER_VAPOR,RCON,G_F)
   CALL THERMO_TABLE_LIQUID (I,CP_L,H_V,T_R,T_M,T_B,WATER_VAPOR,FUEL,DENSITY,MU_LIQUID,K_LIQUID,BETA_LIQUID)
   IF (I==1) THEN
      CP_G_O = CP_G
      CP_L_O = CP_L
   ENDIF
