...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   TIME_STEP_LOOP: DO ITER=1,N_ITER
      ...
      WALL_SEARCH: IF (.NOT.HIT_SOLID .AND. LP%CFACE_INDEX==0 .AND. &
         ...
         ELSEIF (HIT_SOLID .AND. ICF>0) THEN
...
USE GEOMETRY_FUNCTIONS, ONLY: GET_FACE_VOLUMES
...
REAL(EB) :: UBAR,VBAR,WBAR,UREL,VREL,WREL,QREL,RHO_G,TMP_G,MU_FILM, &
            ...
            GX_LOC,GY_LOC,GZ_LOC,DRAG_MAX(3)=0._EB,K_SGS,U_P,KN,M_DOT,EMBER_DENSITY,EMBER_VOLUME,ACCEL_X,ACCEL_Y,ACCEL_Z,&
            LP_FORCE_X,LP_FORCE_Y,LP_FORCE_Z,FACE_VOLS(0:1,0:1,0:1)
REAL(EB), SAVE :: BETA
INTEGER :: IIX,JJY,KKZ
...
IF (CC_IBM) THEN
   IF (X_WGT>=0.5_EB .AND. FCVAR(IIG_OLD-1,JJG_OLD,KKG_OLD,CC_FGSC,1)==CC_SOLID) X_WGT = 1._EB
   IF (X_WGT< 0.5_EB .AND. FCVAR(IIG_OLD  ,JJG_OLD,KKG_OLD,CC_FGSC,1)==CC_SOLID) X_WGT = 0._EB
   IF (Y_WGT>=0.5_EB .AND. FCVAR(IIG_OLD,JJG_OLD-1,KKG_OLD,CC_FGSC,2)==CC_SOLID) Y_WGT = 1._EB
   IF (Y_WGT< 0.5_EB .AND. FCVAR(IIG_OLD,JJG_OLD  ,KKG_OLD,CC_FGSC,2)==CC_SOLID) Y_WGT = 0._EB
   IF (Z_WGT>=0.5_EB .AND. FCVAR(IIG_OLD,JJG_OLD,KKG_OLD-1,CC_FGSC,3)==CC_SOLID) Z_WGT = 1._EB
   IF (Z_WGT< 0.5_EB .AND. FCVAR(IIG_OLD,JJG_OLD,KKG_OLD  ,CC_FGSC,3)==CC_SOLID) Z_WGT = 0._EB
ENDIF
...
LP_FORCE_X = ACCEL_X/LP%RVC
LP_FORCE_Y = ACCEL_Y/LP%RVC
LP_FORCE_Z = ACCEL_Z/LP%RVC

CALL GET_FACE_VOLUMES(NM,1,IIG_OLD-1,JJY,KKZ,FACE_VOLS)
CALL PARTICLE_TO_MESH(LP_FORCE_X,FVX_D,FACE_VOLS,IIG_OLD-1,JJY,KKZ,X_WGT2,Y_WGT,Z_WGT)

CALL GET_FACE_VOLUMES(NM,2,IIX,JJG_OLD-1,KKZ,FACE_VOLS)
CALL PARTICLE_TO_MESH(LP_FORCE_Y,FVY_D,FACE_VOLS,IIX,JJG_OLD-1,KKZ,X_WGT,Y_WGT2,Z_WGT)

CALL GET_FACE_VOLUMES(NM,3,IIX,JJY,KKG_OLD-1,FACE_VOLS)
CALL PARTICLE_TO_MESH(LP_FORCE_Z,FVZ_D,FACE_VOLS,IIX,JJY,KKG_OLD-1,X_WGT,Y_WGT,Z_WGT2)
...
USE MEMORY_FUNCTIONS, ONLY: REALLOCATE_STORAGE_ARRAYS,NULLIFY_PARTICLE
INTEGER :: MM,NOM
