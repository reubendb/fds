...
USE COMPLEX_GEOMETRY, ONLY : CC_SOLID,CC_FGSC,DEFINE_CC_INTERP_WEIGHTS,GET_CC_FACE_VOLUMES
REAL(EB) :: UBAR,VBAR,WBAR,UREL,VREL,WREL,QREL,RHO_G,TMP_G,MU_FILM, &
            ...
            EMBER_DENSITY,EMBER_VOLUME,ACCEL_X,ACCEL_Y,ACCEL_Z,LP_FORCE,FACE_VOLS(2,2,2)
...
IF (CC_IBM) THEN
   CALL DEFINE_CC_INTERP_WEIGHTS(LP,XI,YJ,ZK,IIX,JJY,KKZ,IIG_OLD,JJG_OLD,KKG_OLD,&
...
DO AXIS=1,3
   IL = IIX; JL = JJY; KL = KKZ; X_WGT3 = X_WGT; Y_WGT3 = Y_WGT; Z_WGT3 = Z_WGT
   IF (AXIS == IAXIS) THEN
   ...
   IF(CC_IBM) THEN; CALL GET_CC_FACE_VOLUMES(NM,AXIS,IL,JL,KL,IIG_OLD,JJG_OLD,KKG_OLD,FACE_VOLS)
   ELSE; CALL GET_FACE_VOLUMES(NM,AXIS,IL,JL,KL,FACE_VOLS); ENDIF
