...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   IF (NLP>0 .AND. SOLID_PARTICLES) THEN
      DO IP = 1,NLP
         ...
         KAPPA_PART_SINGLE = 0.25_EB*LP%PWT*B1%AREA*RDX(IID)*RRN(IID)*RDY(JJD)*RDZ(KKD)*B1%EMISSIVITY
   ...
   ELSE WIDE_BAND_MODEL_IF
      ...
      RTE_SOURCE_CORRECTION_IF: IF (RTE_SOURCE_CORRECTION) THEN ! default RTE_SOURCE_CORRECTION=.TRUE.
         ...
         !$OMP PARALLEL PRIVATE(RAD_Q_SUM_PARTIAL,KFST4_SUM_PARTIAL) SHARED(RAD_Q_SUM,KFST4_SUM)
   ...
   INTENSITY_UPDATE: IF (UPDATE_INTENSITY) THEN
      ...
      UPDATE_LOOP: DO I_UIID = 1,N_UPDATES
         ...
         ANGLE_LOOP: DO N = NSTART,NEND,NSTEP  ! Sweep through control angles
            ...
            IF (CC_IBM) THEN
               ...
               DLA = (/DLX(N),DLY(N),DLZ(N)/)
               CFACE_LOOP2: DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
                  CFA => CFACE(ICF)
      ...
      DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         ...
         B1 => BOUNDARY_PROP1(WC%B1_INDEX)
         SF => SURFACE(WC%SURF_INDEX)
         EFLUX = EVALUATE_RAMP(T,SF%RAMP(TIME_EFLUX)%INDEX,TAU=SF%RAMP(TIME_EFLUX)%TAU)*SF%EXTERNAL_FLUX
