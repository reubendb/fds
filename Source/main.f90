...
USE VTK_FDS_INTERFACE, ONLY : WRITE_PARAVIEW_STATE_FILE
...
INTEGER, ALLOCATABLE, DIMENSION(:) :: REQ,REQ1,REQ2,REQ3,REQ4,REQ5,REQ7,REQ6,REQ14,REQ15
#else
...
IF (MY_RANK==0 ) THEN
   IF (PARAVIEW_PROJECT) THEN
      CALL WRITE_PARAVIEW_STATE_FILE(NMESHES)
   ENDIF
ENDIF
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   IF (MY_RANK > 0 ) THEN
      CALL MPI_SEND ( MESHES(NM)%N_STRINGS, 1, MPI_INTEGER, 0, 1, &
                      MPI_COMM_WORLD, IERR )
      IF (MESHES(NM)%N_STRINGS > 0 ) THEN
        ...
        DO N=1,MESHES(NM)%N_STRINGS
          ...
          END ASSOCIATE
        END DO
        !CALL MPI_SEND( MESHES(NM)%STRING(1), &
        !               MESHES(NM)%N_STRINGS*MESH_STRING_LENGTH, &
        !               MPI_CHARACTER, 0, NM, MPI_COMM_WORLD,IERR)
        CALL MPI_SEND( STRING_BUF, MESHES(NM)%N_STRINGS*MESH_STRING_LENGTH, &
                       MPI_CHARACTER, 0, NM, MPI_COMM_WORLD,IERR)
   ...
   OTHER_MESH_LOOP: DO NOM=2,NMESHES
      IF (PROCESS(NOM)>0) THEN
         ...
         IF (N_STRINGS_DUM>0) THEN
            ALLOCATE(CHARACTER(LEN=N_STRINGS_DUM*MESH_STRING_LENGTH) :: &
                     STRING_BUF )
            CALL MPI_RECV(STRING_BUF,N_STRINGS_DUM*MESH_STRING_LENGTH, &
...
INTEGER  :: LENGTH(2)
#ifdef WITHOUT_MPIF08
INTEGER :: DTYPES(2),MESH_STRUCT_TYPE
#else
